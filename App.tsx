      ...
      try {
        ...
        if (data.error) {
          ...
          try {
            ...
            if (isAndroid) {
              // ── Android path ─────────────────────────────────────────────
              // Hand the absolute URL + filename to the Android Download
              // Manager via the JavascriptInterface bridge if available,
              // otherwise open in the system browser which triggers the
              // native download manager automatically.
              // Either way the app stays open and no blob is loaded into RAM.
              const androidBridge = (window as any).AndroidInterface;
              if (androidBridge?.downloadFile) {
                androidBridge.downloadFile(absoluteUrl, filename);
              } else {
                // _system opens URL in the default Android browser/download manager
                // without navigating the WebView itself
                (window as any).open(absoluteUrl, '_system');
              }
            } else {
              // ── Browser / Desktop path ───────────────────────────────────
              // Fetch as blob so the browser's built-in save dialog appears
              // without navigating away from the page.
              setDownloadText('Fetching file...');
